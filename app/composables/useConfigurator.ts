...
export type LeafFinish = import('~/data/doorFinishes').LeafFinish
...
export function useConfigurator() {
  ...
  const leafFinish = useState<LeafFinish>('cfg-leaf', () => 'paint')
  ...
  async function openProduct(slug: string, next?: { edge?: ConfigEdge, swing?: ConfigSwing, leafFinish?: LeafFinish }) {
    ...
    if (next?.leafFinish) leafFinish.value = next.leafFinish
  ...
  return { selectedSlug, edge, swing, leafFinish, openProduct }
}
